...
// JEI plugin entry point. On Fabric JEI discovers this via the jei_mod_plugin entrypoint in
// fabric.mod.json (the @JeiPlugin annotation alone is not enough on Fabric); it only loads when JEI is
// present, so JEI stays an optional soft dependency. The annotation is kept for parity with NeoForge
@JeiPlugin
public class BlockVariantSwapperJeiPlugin implements IModPlugin {
    ...
    //
    // Read straight from BlockVariantConfig rather than BlockVariantManager: JEI registers its recipes
    // at client resource load, BEFORE any world is joined, but the manager is only populated on the
    // server-data reload (world load or /reload). Sourcing from the config, which loads purely from
    // bundled jar resources plus user files, means the category is populated regardless of world state.
