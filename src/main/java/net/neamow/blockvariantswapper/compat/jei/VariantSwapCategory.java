...
public class VariantSwapCategory implements IRecipeCategory<VariantSwapRecipe> {
    ...
    public Text getTitle() {
        return Text.translatable("gui.blockvariantswapper.jei.category");
    }
    ...
    @Override
    public void draw(VariantSwapRecipe recipe, IRecipeSlotsView recipeSlotsView, DrawContext guiGraphics, double mouseX, double mouseY) {
        TextRenderer font = MinecraftClient.getInstance().textRenderer;

        // Base block label to the right of the base slot, vertically centred against it
        Text baseLabel = Text.translatable("gui.blockvariantswapper.jei.base");
        int labelY = BASE_SLOT_Y + (SLOT - font.fontHeight) / 2;
        guiGraphics.drawText(font, baseLabel, BASE_LABEL_X, labelY, 0x555555, false);

        // Keybind hint below the base row
        Text hint = Text.translatable("gui.blockvariantswapper.jei.hint", ModKeyBinding.swapKey.getBoundKeyLocalizedText());
        guiGraphics.drawText(font, hint, PADDING, HINT_Y, 0x555555, false);
    }
    ...
    public Identifier getRegistryName(VariantSwapRecipe recipe) {
        return net.minecraft.registry.Registries.ITEM.getId(recipe.base());
    }
}
