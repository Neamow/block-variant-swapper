...
// Discovered automatically via the @JeiPlugin annotation, so doesn't run (or even load) unless JEI is present
@JeiPlugin
public class BlockVariantSwapperJeiPlugin implements IModPlugin {
    ...
    // Built once per plugin instance and reused by both registerRecipes and registerRecipeCatalysts
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        // One entry per variant, sourced from the config (see getRecipes)
    ...
    // Lazily build and cache the entries, so the two register* passes share one load
    ...
    // Build one entry per variant, pairing each with its family's base block
    //
