...
// Only loads it when JEI is present
@JeiPlugin
public class BlockVariantSwapperJeiPlugin implements IModPlugin {
    private static final ResourceLocation PLUGIN_UID = BlockVariantSwapper.id("jei");
    ...
    //
    // Read straight from BlockVariantConfig rather than BlockVariantManager: JEI registers its recipes at client resource load, BEFORE
    // any world is joined, but the manager is only populated on the server-data reload (world load or /reload)
    ...
    private static Item resolveItem(String id) {
        ...
        return BuiltInRegistries.ITEM.get(parsed);
    }
}
