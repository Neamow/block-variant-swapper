...
//  - The on-disk config folder (config/blockvariantswapper/) is user territory.
...
public class BlockVariantConfig {
    ...
    // Loaded from the jar on every (re)load; never written to the config folder
    ...
    // Ensure the (empty) user config directory exists so players can drop overrides in
    ...
    public static void load() {
        ...
        // 1. Load bundled defaults from the jar with union semantics
        //    If two default files contribute to the same base block (e.g. a mod adds shapes
        //    to a vanilla base), their variant lists are combined rather than replaced
    ...
    // Union merge: variant lists for a shared base block are combined (deduplicated, order-preserving)
    ...
    // Read a single user JSON file and apply its groups on top of the map (replace semantics)
    ...
    // Load a config file bundled inside the mod's own jar resources
    ...
    private static Path getConfigDir() {
        return FMLPaths.CONFIGDIR.get().resolve(BlockVariantSwapper.MOD_ID);
    }
